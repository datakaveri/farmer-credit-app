...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // std::ifstream jsonFile("/home/ubuntu/p3dx-infrastructure/client_data.json");
        // if (!jsonFile.is_open()) {
        ...
        // json jsonData;
        // jsonFile >> jsonData; // Parse the JSON data

        // std::string client_data="hello world";
        json j;
        std::string public_key=readFromFile("../../keys/public_key.pem");
        std::string tpm_values=readFromFile("../../keys/pcr_values.json");
        j["public key"] = public_key;
        j["tpm_values"] = tpm_values;
        //std::string customJsonData = jsonData.dump();
        //std::string client_payload_str = "{\"public key\":\"" + public_key + "\", \"tpm_values\":\"" + tpm_values + "\"}"; // nonce is optional
        std::string client_payload_str = j.dump();
        // std::cout << "Client payload string: " << client_payload_str << std::endl;

        unsigned char* client_payload = (unsigned char*)client_payload_str.c_str();
        params.client_payload = client_payload;
