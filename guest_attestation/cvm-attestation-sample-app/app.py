...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        p = subprocess.Popen(['sudo ./AttestationClient -o token'], shell=True,
        ...
        if  l == 3:
            ...
            print (decoded)
            # decoded = jwt.decode(g, public_key, algorithms=['RS256'], options={"verify_signature": True})
            # print("JWT is verified and decoded:", decoded)
            ...
            self.wfile.write(json.dumps({"error": f"JWT was not formed properly {g}"}).encode("utf-8"))
